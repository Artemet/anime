/******/ (() => { // webpackBootstrap
var __webpack_exports__ = {};
/*!******************************!*\
  !*** ./resources/js/home.js ***!
  \******************************/
//toggle_click
window.toggle_click = function (back) {
  var get_circle = document.querySelector(".toggle-circle");
  get_circle.style.right = "2px";
  var get_toggle = document.querySelector(".toggle");
  get_toggle.style.background = "#666";
  var get_chat_container = document.querySelector(".chat_container");
  get_chat_container.style.display = "none";
  toggle.get_circle.classList = "no_display";
}; //proverka_in


var get_come_in = document.querySelector(".come_in");
var get_text_in = document.querySelector(".text_in");
console.log(get_come_in.style.display);

if (get_come_in.style.display == "block") {
  get_text_in.style.display = "none";
} //menu


window.open_sub = function (open) {
  var get_sub = open.querySelector(".navigation_container");
  open.style.color = "red";
};
/******/ })()
;